...
public class ListEventLog extends BaseEventLog implements Cloneable, Serializable {
    ...
    public Calendar getTimeCalendar(DateTimeDataProvider convert, ListEventLogTimeType timeType) {
        return convert.convertServerTimeToCalendar(getTimeType(timeType), true);
    }

    public boolean setTimeCalendar(DateTimeDataProvider convert, ListEventLogTimeType timeType, Calendar cal) {
        return setTimeType(timeType, convert.convertCalendarToServerTime(cal, true));
    }

    public String getTimeFormmat(DateTimeDataProvider convert, ListEventLogTimeType timeType, DateTimeDataProvider.DATE_TYPE type) {
        Calendar cal = getTimeCalendar(convert, timeType);
        return convert.convertCalendarToFormatter(cal, type);
    }

    public boolean setTimeFormmat(DateTimeDataProvider convert, ListEventLogTimeType timeType, DateTimeDataProvider.DATE_TYPE type, String src) {
