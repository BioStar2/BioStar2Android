...
public class SwipyRefreshLayout extends ViewGroup {
    ...
    private boolean mEnableBottom = true;
    ...
    private boolean mTwicesetrefrsh;
    ...
    Runnable mRunsetRefreshing = new Runnable() {
        @Override
        public void run() {
            setRefreshing(true);
        }
    ...
    public void setRefreshing(boolean refreshing) {
        ...
        if (height == 0 && refreshing && !mTwicesetrefrsh) {
            mTwicesetrefrsh = true;
            post(mRunsetRefreshing);
            return;
        }
    ...
    public void onRefresh(SwipyRefreshLayoutDirection direction, boolean isDataRefresh) {
        if (mRefreshing != true) {
            setRawDirection(direction);
            if (mListener != null && isDataRefresh) {
                mListener.onRefresh(mDirection);
            }
            setRefreshing(true);
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (mDirection) {
            case BOTTOM:
                if (!isEnabled() || mReturningToStart || canChildScrollDown() || mRefreshing || !mEnableBottom) {
            ...
            case TOP:
            default:
                if (!isEnabled() || mReturningToStart || canChildScrollUp() || mRefreshing || !mEnableTop) {
                    // Fail fast if we're not in a state where a swipe is possible
                    return false;
                }
                break;
...
//    }
    ...
    public void setEnableBottom(boolean b) {
        mEnableBottom = b;
    }

    public void setEnableTop(boolean b) {
        mEnableTop = b;
    }
